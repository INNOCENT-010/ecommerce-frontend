// app/components/home/BestSellersGrid.tsx - FIXED VERSION
'use client';
...
function BestSellersGrid() {
  ...
  // Calculate slides count based on products
...
const convertToProductFormat = (product: BestSellerProduct): Product => {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 h-[65vh] min-h-[500px]">
        {[...Array(4)].map((_, index) => (
  ...
  return (
    <div className="relative">
      ...
      <button
        onClick={prevSlide}
        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-6 p-4 bg-black/0 hover:bg-black/5 rounded-full transition-all duration-300 z-30 group"
        aria-label="Previous slide"
      >
        <div className="p-3 bg-white/95 backdrop-blur-sm rounded-full group-hover:bg-white transition-all shadow-xl border border-gray-200">
      ...
      <button
        onClick={nextSlide}
        className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-6 p-4 bg-black/0 hover:bg-black/5 rounded-full transition-all duration-300 z-30 group"
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-4">
          ...
          {getCurrentSlideProducts().length < productsPerSlide && 
            Array.from({ length: productsPerSlide - getCurrentSlideProducts().length }).map((_, index) => (
              <div 
                key={`empty-${index}`} 
                className="col-span-1"
              >
                <div className="h-[65vh] min-h-[400px] opacity-0 pointer-events-none"></div>
              </div>
            ))
          }
        </div>
      ...
      {/* Products Count & Current Slide */}
      {products.length > 0 && (
        <div className="text-center mt-4 text-sm text-gray-600">
          Showing {Math.min(products.length, productsPerSlide)} of {products.length} bestsellers
      ...
      <div className="text-center mt-8">
        <Link
          href="/products"
          className="inline-flex items-center justify-center border border-black text-black px-8 py-3 font-medium hover:bg-black hover:text-white transition-all text-sm rounded-lg group"
        >
