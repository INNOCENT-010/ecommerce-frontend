...
// Create rows with 3 products per row - MOBILE ADAPTED
const createRows = (products: Product[], firstCardBigger: boolean, isMobile: boolean) => {
  if (isMobile) {
    // On mobile, create rows of 2 products instead of 3
    const rows = [];
    for (let i = 0; i < products.length; i += 2) {
      const rowProducts = products.slice(i, i + 2);
      rows.push({
        type: 'equal' as const, // On mobile, always equal cards
    ...
    // Desktop: original 3 products per row
    const rows = [];
...
export default function ProductGrid({ 
  ...
  // Configuration
  const PRODUCTS_PER_PAGE = isMobile ? 12 : 36;
  const MAX_PAGE_BUTTONS = isMobile ? 3 : 5;
  const ROW_HEIGHT = isMobile ? 'auto' : '85vh';
  ...
  // Create rows from displayed products
  const rows = createRows(displayedProducts, firstCardBigger, isMobile);
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-8">
          {[...Array(4)].map((_, rowIndex) => (
            <div key={rowIndex} className={`grid grid-cols-1 ${isMobile ? 'grid-cols-2' : 'md:grid-cols-3'} gap-4 md:gap-6 animate-pulse`}>
              {[...Array(isMobile ? 2 : 3)].map((_, colIndex) => (
                <div key={colIndex}>
                  <div className={`bg-gray-200 rounded-lg ${isMobile ? 'aspect-[3/4]' : 'h-[85vh]'} mb-3`}></div>
  ...
  return (
    <div className="space-y-8">
      {showFilters && (
        <div 
          ref={stickyRef}
          className={`${isMobile ? 'sticky top-16' : 'sticky bg-white'} transition-all duration-300 ${
            isSticky 
              ? 'top-0 shadow-lg border-b border-gray-200 py-4 px-4 -mx-4' 
              : 'border-b border-gray-200 pb-4'
          }`}
        >
          <div className={`flex flex-col ${isMobile ? 'gap-3' : 'md:flex-row md:items-center justify-between gap-4'}`}>
            ...
            <div className={`flex ${isMobile ? 'flex-wrap gap-2' : 'items-center gap-2'}`}>
              {isMobile ? (
                ...
                <button
                  onClick={() => {
                    // Simple mobile filter toggle
                    const filterButton = document.querySelector('.filter-dropdown-button');
                    if (filterButton) {
                      filterButton.classList.toggle('hidden');
                    }
                  }}
                  className="flex items-center gap-2 px-4 py-2 rounded-full bg-black text-white text-sm font-medium"
                >
                  <Filter size={14} />
      ...
      {activeFilterCount > 0 && (
        <div className={`${isMobile ? 'px-4' : 'sticky z-30 bg-white pt-2'} transition-all duration-300 ${
          isSticky && !isMobile ? 'top-14' : 'top-0'
        } ${transitioning ? 'opacity-50' : 'opacity-100'}`}>
      ...
      <div className={`space-y-8 pt-2 transition-opacity duration-300 ${
        ...
        {products.length === 0 ? (
          ...
          rows.map((row, rowIndex) => {
            if (isMobile) {
              // Mobile layout: 2 equal cards per row
              return (
                <div
                  key={rowIndex}
                  className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4"
                >
              ...
              // Desktop equal layout: 3 equal cards per row
              return (
                <div
                  key={rowIndex}
              ...
              // Desktop first card bigger layout
              return (
                <div
                  key={rowIndex}
            ...
            <div className="flex items-center gap-1">
              {pageNumbers.map(page => (
                <button
                  ...
                  className={`${isMobile ? 'w-8 h-8' : 'w-10 h-10'} flex items-center justify-center rounded-lg border text-sm font-medium transition-all ${
                    currentPage === page
                      ? 'bg-black text-white border-black'
              ...
              {totalPages > MAX_PAGE_BUTTONS && currentPage < totalPages - 2 && (
                <>
                  ...
                  <button
                    ...
                    className={`${isMobile ? 'w-8 h-8' : 'w-10 h-10'} flex items-center justify-center rounded-lg border text-sm font-medium border-gray-300 text-gray-700 hover:bg-gray-50`}
                  >
                    {totalPages}
                  </button>
                </>
